...
class WalletMsigna(object):
    ...
    @classmethod
    def load_from_filename(cls, wallet_filename, options):
        ...
        try:
            if "args" in globals() and options.msigna_keychain:  # args is not defined during unit tests
                wallet_cur = wallet_conn.execute(select + b" WHERE name LIKE '%' || ? || '%'", (options.msigna_keychain,))
            else:
